...
interface EmbeddedPerson { personId: string; name: string; idx: number[]; val: number[]; }     // sparse, L2-normalized
...
interface LayoutNode { id: string; name: string; x: number; y: number; cluster: number; size: number; }
interface LayoutNode3D { id: string; name: string; x: number; y: number; z: number; cluster: number; size: number; } // static projector positions
