...
/**
 * scripts/scratch/submit-top25-activity.cjs
 *
 * Dedicated script to submit a custom Autodesk Data Connector request for the top 25
 * highest-activity projects, poll for job success, download the zipped output,
 * stream-ingest all activities into the database, and update the progressive backfill progress.
 *
 * Uses 3-legged user credentials and consumes exactly 1 Data Connector API quota request.
 */
...
const START_DATE = "2026-04-22T00:00:00.000Z";
const END_DATE = "2026-05-22T23:59:59.999Z";
...
function log(...args) { console.log(`[dc-top25 ${ts()}]`, ...args); }
function logErr(...args) { console.error(`[dc-top25 ${ts()}]`, ...args); }
...
async function submitRequest(accountId, accessToken, projectIds) {
  const body = {
    description: `Dashboard Top25 activities ${START_DATE} to ${END_DATE}`,
