...
function cleanName(line) {
  ...
  // Match patterns like "00 D Name" or "35.1 D Name" or "47 Name"
...
// Normalize a string for comparison (lowercase, strip accents, remove non-alphanumeric except spaces)
...
// Return tokens that are meaningful
const STOP_WORDS = new Set(["d", "ed", "ti", "tis", "and", "center", "fase", "phase", "oficinas", "infra", "infraestructura", "ampliacion", "expansion", "n", "gate", "park", "life", "inv", "inventario", "nave", "business", "building", "de", "y", "el", "la", "los", "las"]);
...
async function main() {
  ...
  try {
    ...
    for (const l of lines) {
      ...
      if (cleaned) {
        targets.push({ raw: l, clean: cleaned, norm: normalizeStr(cleaned), tokens: getTokens(cleaned) });
      }
    }

    console.log(`Parsed ${targets.length} target project names from prompt.\n`);
    ...
    console.log(`Database has ${dbProjects.length} projects in AccProject.`);
    ...
    for (const target of targets) {
      // Try exact or substring match on normalized names
      ...
      // If no match, try token-based matching
      if (matches.length === 0 && target.tokens.length > 0) {
        matches = dbProjects.filter(p => {
          const pNorm = normalizeStr(p.name);
          // Check if ALL target tokens are in the project name
          return target.tokens.every(token => pNorm.includes(token));
        });
      }

      // If still no match, try matching if AT LEAST 2 tokens match or if it's a 1-token target, that token matches
      if (matches.length === 0 && target.tokens.length > 0) {
        matches = dbProjects.filter(p => {
          const pNorm = normalizeStr(p.name);
          const matchedTokens = target.tokens.filter(token => pNorm.includes(token));
      ...
      if (matches.length > 0) {
        // If there are multiple matches, pick the one that has the closest length or best token overlap
        let bestMatch = matches[0];
        if (matches.length > 1) {
          // Sort by name similarity (e.g. difference in length)
          matches.sort((a, b) => {
            const diffA = Math.abs(a.name.length - target.clean.length);
            const diffB = Math.abs(b.name.length - target.clean.length);
            return diffA - diffB;
          });
          bestMatch = matches[0];
        }
        matched.push({ target, bestMatch, allMatches: matches });
        matchedProjectIds.add(bestMatch.id);
      } else {
        unmatched.push(target);
      }
    }

    console.log(`=== SMART MATCH SUMMARY ===`);
    console.log(`Successfully matched to DB: ${matched.length}`);
    console.log(`Unmatched: ${unmatched.length}`);
    console.log(`Distinct DB Project IDs matched: ${matchedProjectIds.size}`);

    console.log(`\n=== MATCHED SAMPLES (FIRST 20) ===`);
    matched.slice(0, 20).forEach(m => {
      console.log(`- Target: "${m.target.clean}" matched: "${m.bestMatch.name}" (${m.bestMatch.id}) [from ${m.allMatches.length} candidates]`);
    });

    console.log(`\n=== UNMATCHED SAMPLES (FIRST 20) ===`);
    unmatched.slice(0, 20).forEach(u => {
      console.log(`- "${u.clean}" (tokens: ${u.tokens.join(", ")})`);
    });
