...
async function main() {
  ...
  try {
    ...
    for (const l of lines) {
      ...
      if (cleaned) {
        targetNames.push({ raw: l, clean: cleaned });
      }
    }

    console.log(`Parsed ${targetNames.length} target project names from prompt.\n`);

    // Fetch all active/inactive projects in local database
    ...
    const dcProjects = await prisma.accDcProject.findMany({
      select: { id: true, name: true }
    });
    console.log(`Database has ${dcProjects.length} projects in AccDcProject.\n`);

    const matched = [];
    const unmatched = [];

    for (const target of targetNames) {
      // Find a project in DB whose name contains the cleaned target name, or vice versa
      const matches = dbProjects.filter(p => {
        const pName = p.name ? p.name.toLowerCase() : "";
        const tName = target.clean.toLowerCase();
        return pName === tName || pName.includes(tName) || tName.includes(pName);
      });

      if (matches.length > 0) {
        matched.push({ target, matches });
      } else {
    ...
    console.log(`Total Target Projects from User's list: ${targetNames.length}`);
    console.log(`Successfully matched to DB: ${matched.length}`);
    console.log(`Unmatched: ${unmatched.length}`);

    console.log(`\n=== MATCHED SAMPLES (FIRST 15) ===`);
    matched.slice(0, 15).forEach(m => {
      console.log(`- Target: "${m.target.clean}" matched: ${m.matches.map(p => `"${p.name}" (${p.id})`).join(", ")}`);
    });
