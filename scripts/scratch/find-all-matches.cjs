...
async function main() {
  ...
  try {
    ...
    console.log(`Checking search matches in Database:\n`);
    const mappedIds = new Set();
    const mappings = [];

    // Keywords to search for Yazaki, Salinas, Scrum, Lunaris, etc.
    const specialKeywords = ["yazaki", "salinas", "scrum", "lunaris", "espanol", "trojano", "mexlog", "cienega", "artha", "dewa", "escobedo"];

    // Let's do a smart token match for each project
    for (const target of targets) {
      // Clean up target name tokens
      const tokens = target.norm.split(" ").filter(t => t.length > 2 && t !== "project" && t !== "prospect" && t !== "park");

      // Match logic
      let matches = dbProjects.filter(p => {
        const pNorm = normalizeStr(p.name);
        return pNorm.includes(target.norm) || target.norm.includes(pNorm);
      });

      if (matches.length === 0 && tokens.length > 0) {
        matches = dbProjects.filter(p => {
          const pNorm = normalizeStr(p.name);
      ...
      if (matches.length === 0 && tokens.length > 0) {
        matches = dbProjects.filter(p => {
          const pNorm = normalizeStr(p.name);
          const matched = tokens.filter(token => pNorm.includes(token));
          if (tokens.length === 1) return matched.length === 1;
          // If we have at least 2 tokens matched
          return matched.length >= Math.max(2, Math.floor(tokens.length * 0.7));
        });
      }

      if (matches.length > 0) {
        // Find best match (closest length)
        matches.sort((a, b) => Math.abs(a.name.length - target.clean.length) - Math.abs(b.name.length - target.clean.length));
        const best = matches[0];
        mappedIds.add(best.id);
        mappings.push({ target: target.clean, matchedName: best.name, id: best.id });
      }
    }

    // Also: let's automatically add ANY project in DB that starts with "mty" or contains "monterrey" or starts with "mty-" or "mty " or "mty_"
    const mtyExplicit = dbProjects.filter(p => {
      const n = p.name.toLowerCase();
      return n.startsWith("mty") || n.includes("monterrey") || n.startsWith("mty-") || n.startsWith("mty ");
    });

    mtyExplicit.forEach(p => {
      if (!mappedIds.has(p.id)) {
        mappedIds.add(p.id);
        mappings.push({ target: "[Explicit MTY Prefix Search]", matchedName: p.name, id: p.id });
      }
    });

    console.log(`Matched total distinct IDs: ${mappedIds.size}`);
    
    // Filter the final list to exclude non-MTY projects and demo/vdc projects
    const finalIds = [];
    const excludedCount = { division: 0, test: 0 };
    for (const id of mappedIds) {
      const p = dbProjects.find(x => x.id === id);
      if (!p) {
        finalIds.push(id);
        continue;
      }
      const lowerName = p.name.toLowerCase();
      const hasOtherDivision = lowerName.includes("mxl") || 
                               lowerName.includes("tij") || 
                               lowerName.includes("qro") || 
                               lowerName.includes("cdmx");
      const isDemoOrVdc = lowerName.includes("demo") ||
      ...
      if (hasOtherDivision) {
        excludedCount.division++;
        console.log(`Excluding project ID ${id} ("${p.name}") due to other division keywords (MXL/TIJ/QRO/CDMX).`);
      } else if (isDemoOrVdc) {
        excludedCount.test++;
        console.log(`Excluding project ID ${id} ("${p.name}") due to demo/vdc/test keywords.`);
      } else {
        finalIds.push(id);
      }
    }
    console.log(`Excluded ${excludedCount.division} other division projects and ${excludedCount.test} demo/test projects.`);
    console.log(`Final filtered Monterrey projects: ${finalIds.length}`);

    // Write the list of distinct IDs to a file
    const fs = require("fs");
    fs.writeFileSync("lib/acc/mty-allowlist.json", JSON.stringify(finalIds, null, 2));
    console.log("Wrote filtered lib/acc/mty-allowlist.json");
