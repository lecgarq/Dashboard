#!/usr/bin/env node
/**
 * scripts/scratch/extract-top-members.cjs
 *
 * Systematic bulk extraction of project members (users, roles, company, products)
 * for the highest-activity unextracted projects using standard 2-legged tokens.
...
function log(...args) { console.log(`[top-members-extract ${ts()}]`, ...args); }
function logErr(...args) { console.error(`[top-members-extract ${ts()}]`, ...args); }
...
async function main() {
  ...
  try {
    log("=== STARTING BULK EXTRACTION OF TOP UNEXTRACTED PROJECTS MEMBERS ===");
    ...
    // Query top active non-MTY projects that have 0 members in the DB
    log("Querying top active unextracted projects ordered by activity count...");
    const topProjects = await prisma.$queryRaw`
      ...
      ORDER BY COALESCE(act.act_count, 0) DESC
      LIMIT 25
    `;

    log(`Found ${topProjects.length} active unextracted projects ordered by activity count.`);

    if (topProjects.length === 0) {
      log("No active unextracted projects found. Exiting.");
      return;
    }
    ...
    await runWithConcurrency(5, topProjects, async (project) => {
      try {
    ...
    log(`\n=== BULK TOP MEMBERS EXTRACTION COMPLETE ===`);
