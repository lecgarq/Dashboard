...
- [x] **SCALE-01**: A feasibility spike **measures, on the workshop machine**, before any
...
- [x] **SCALE-02**: Activity node data reaches the client as **binary columnar payloads**
...
- [x] **ACT-01**: One graph node per extracted activity event at the shipped ladder rung —
...
- [x] **ACT-02**: Every activity node **inherits its author's properties** — role (via the
...
- [x] **ACT-03**: The activity universe **fully replaces** the user×project instance graph
...
- [x] **ACT-04**: Hover/click on an activity node shows the event honestly (verb, object
...
- [x] **EMB-07**: An offline pipeline computes 2D positions for the full activity corpus —
...
- [x] **DIM-07**: The dimension slider surface (group-by / color-by / strength sliders)
...
- [x] **PERF-07**: Slider morph, grouping transitions, and ambient life run at the shipped
...
- [x] **TIME-01**: A temporal scrubber filters (and can animate) the graph by month across
...
- [x] **REND-04**: **Hard perf gate, measured last:** Tier-0 with ambient life + the
...
- [x] **E2E-03**: `tests/e2e/acc-dc-graph.spec.ts` (+ lasso spec) is re-baselined to the
