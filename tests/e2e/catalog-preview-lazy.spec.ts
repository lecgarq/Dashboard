...
const ACTION_SLIDER = "Issue Create";
...
test("action dimensions become live sliders only after Dimensions opens", async ({ page }) => {
  ...
  expect(containsGeneratedActions(initialCoverage), "generated action code is absent on Layout first paint").toBe(false);
  ...
  expect(containsGeneratedActions(catalogCoverage), "generated action code loads after the Dimensions click").toBe(true);
  ...
  await search.fill(ACTION_SLIDER);
  await expect(page.getByText(ACTION_SLIDER, { exact: true })).toBeVisible();
  await expect(page.getByRole("slider", { name: `${ACTION_SLIDER} thumb` })).toBeVisible();
