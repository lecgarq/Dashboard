...
import type { SimWiki } from "@prisma/client";
...
function findSectionIndex(sections: SimWiki[], incomingSection: string): number {
...
function applyWikiUpsert(
  current: SimWiki[] | undefined,
  payload: Extract<SimStreamEvent, { type: "wiki-upsert" }>,
...
function applyWikiDelete(
  current: SimWiki[] | undefined,
  payload: Extract<SimStreamEvent, { type: "wiki-deleted" }>
) {
  if (!current) return current;
  if (current.length > 0 && current[0].projectId !== payload.projectId) return current;
...
function applyWikiStatus(
  current: SimWiki[] | undefined,
  payload: Extract<SimStreamEvent, { type: "wiki-status" }>
) {
  if (!current) return current;
  if (current.length > 0 && current[0].projectId !== payload.projectId) return current;
...
export default function SimAutomationPage() {
  ...
  const { data: wikiSections = [] } = trpc.sim.getWikiSections.useQuery();
  ...
  const handleSimEvent = useCallback(
    (payload: SimStreamEvent) => {
      ...
      if (payload.type === "wiki-list-reordered") {
        void utils.sim.getWikiSections.invalidate();
      }
    },
    [utils]
  );

  useEventSource<SimStreamEvent>("/api/sim-updates", handleSimEvent);
  ...
  const upsertWikiSection = trpc.sim.upsertWikiSection.useMutation({
    onSuccess: () => {
       utils.sim.getWikiSections.invalidate();
    }
  });

  const deleteWikiSection = trpc.sim.deleteWikiSection.useMutation({
    onSuccess: () => {
       utils.sim.getWikiSections.invalidate();
    }
  });
