  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        ...
        >
          <span
            className="h-2 w-2 rounded-full transition-transform duration-150 group-hover/tp:scale-125"
            style={{ backgroundColor: TIER_COLOR[value] }}
          />
          <span className={cn("tabular-nums", inherited ? "italic text-muted-foreground" : "text-foreground")}>
            {TIER_SHORT[value]}
          </span>
      ...
      <DropdownMenuContent align="end" className="min-w-[12rem]">
        {FORMA_TIERS.map((t) => (
          <DropdownMenuItem key={t} onSelect={() => onChange(t)} className="cursor-pointer gap-2.5">
            ...
            {t === value && <Check className="h-3.5 w-3.5 text-foreground" />}
          </DropdownMenuItem>
        ))}
      </DropdownMenuContent>
    </DropdownMenu>
