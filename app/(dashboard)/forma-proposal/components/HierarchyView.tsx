...
interface VLink { id: string; sx: number; sy: number; tx: number; ty: number }

// smooth vertical bezier between node centers
...
// Tier menu grouped View / Create / Edit / Manage — mirrors the ACC picker.
...
export function HierarchyView({
  ...
  const { nodes, links, bbox } = useMemo(() => {
    ...
    const ls: VLink[] = laid.links().map((l) => ({
      id: `${l.source.data.id}->${l.target.data.id}`, sx: l.source.x, sy: l.source.y, tx: l.target.x, ty: l.target.y,
    }));
  ...
  const fit = useCallback(() => {
    ...
    const scale = clamp(Math.min(w / (bbox.maxX - bbox.minX + NODE_W), h / (bbox.maxY - bbox.minY + NODE_H)) * 0.9, MIN_SCALE, 1.1);
  ...
  // wheel = pan (scroll), like the folder list; ctrl/⌘+wheel = zoom.
  useEffect(() => {
    const el = containerRef.current;
    if (!el) return;
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <button type="button" className="cursor-pointer outline-none">
                      <OrgNode color={color} label={folder!.name} tierShort={TIER_SHORT[eff!.tier]} inherited={eff!.inherited} open={hasKids && !isCollapsed} />
                    </button>
                  </DropdownMenuTrigger>
